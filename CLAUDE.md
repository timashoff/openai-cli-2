...
## ОБЯЗАТЕЛЬНАЯ ПРОВЕРКА НА ДУБЛИРОВАНИЕ ПЕРЕД РЕАЛИЗАЦИЕЙ
- **ПЕРЕД написанием ЛЮБОГО нового функционала ОБЯЗАТЕЛЬНО проверь существующую кодовую базу** на наличие похожей логики!
- **Многоуровневый поиск существующих решений**:
  - Утилиты и хелперы (`/utils`, `/helpers`, `/lib`, `/common`, `/shared`)
  - Сервисы и менеджеры аналогичных доменов/модулей
  - Компоненты с похожей функциональностью
  - Конфигурационные файлы и файлы констант
  - Существующие third-party зависимости (package.json, requirements.txt)
  - Тестовые файлы (могут содержать полезные паттерны и моки)
  - Middleware, hooks, decorators в смежных модулях
- **Методология поиска**:
  - **Семантический поиск**: искать по ключевым словам назначения, а не только по названиям функций
  - **Grep/ripgrep по паттернам**: регулярные выражения для поиска похожих структур кода
  - **IDE поиск**: использовать "Find in Files" для поиска по комментариям, строкам, константам
  - **Анализ imports/exports**: проследить цепочки зависимостей для выявления существующих решений
  - **Domain analysis**: проверить смежные бизнес-домены на наличие аналогичной логики
  - **Interface/API analysis**: поиск методов с похожими сигнатурами и return types
- **Принцип действий**:
  - **НАЙДЕН похожий код** → адаптировать/расширить существующий, НЕ создавать новый
  - **НАЙДЕНЫ частичные решения** → создать общую абстракцию, объединяющую все случаи использования
  - **НЕ НАЙДЕНО ничего** → только тогда создавать новый код, но проектируя для переиспользования
- **Документирование обнаруженных связей**:
  - При обнаружении дублирования создавать комментарии о связанных решениях
  - Рефакторить найденные дубликаты в общие переиспользуемые модули
  - Обновлять архитектурную документацию при создании новых общих компонентов
- **КРИТИЧЕСКИ ВАЖНО**: Лучше потратить 15 минут на поиск существующего решения, чем создать дублирующий код, который потом придется рефакторить!

...
6. **Проверка на ТЕАТРАЛЬНОСТЬ кода?**
   - Код выглядит функциональным, но является 100% мертвым!
   - Функции, которые никогда не вызываются, но создают иллюзию полезности
   - Переменные и константы, которые определены, но не используются
   - Целые модули или классы, которые импортируются, но их методы никогда не выполняются

### Глубокий анализ импортов и экспортов (ULTRA-THINK режим)

**ОБЯЗАТЕЛЬНАЯ методология для каждого файла при рефакторинге:**

#### Фаза 1: Инвентаризация всех импортов
```javascript
// Для каждого импорта в файле задавай эти вопросы:
import { funcA, funcB, ClassC } from './module.js'
```

**Checklist для каждого импорта:**
- [ ] **Источник**: Открыть файл-источник и понять ЧТО именно импортируется
- [ ] **Фактическое использование**: Найти ВСЕ места в текущем файле, где используется каждый элемент
- [ ] **Театральность**: Есть ли импорты, которые объявлены но НЕ используются?
- [ ] **Логическая связь**: Понять ЗАЧЕМ этот импорт нужен в контексте бизнес-логики
- [ ] **Альтернативы**: Можно ли заменить внешний импорт встроенными возможностями?

#### Фаза 2: Анализ экспортов
```javascript
// Для каждого экспорта проследить его судьбу:
export const myFunction = () => {}
export default SomeComponent
```

**Checklist для каждого экспорта:**
- [ ] **Потребители**: Найти ВСЕ файлы, которые импортируют этот экспорт (grep/search)
- [ ] **Реальное использование**: Убедиться, что импортированный элемент действительно ВЫЗЫВАЕТСЯ
- [ ] **Транзитные импорты**: Выявить файлы, которые импортируют и сразу реэкспортируют (barrel exports)
- [ ] **Мертвые экспорты**: Найти экспорты без потребителей

#### Фаза 3: Трассировка полных цепочек зависимостей

**Пример полной трассировки:**
```
FileA.js imports funcX from FileB.js
→ FileB.js exports funcX (implementation)
→ FileB.js imports utilY from FileC.js
→ FileC.js exports utilY (used in funcX)
→ FileC.js imports constantZ from constants.js
```

**Вопросы для трассировки:**
- Действительно ли ВСЯ цепочка необходима?
- Нет ли промежуточных файлов, которые только перепроводят импорты?
- Можно ли сократить цепочку, импортируя напрямую?
- Есть ли циклические зависимости?

#### Фаза 4: Выявление антипаттернов

**🚩 RED FLAGS - немедленно исправлять:**
- Импорт всего модуля для использования одной функции: `import * as utils from 'utils'`
- Unused imports, оставленные "на всякий случай"
- Re-export chains длиннее 2 уровней
- Импорты из node_modules для функциональности, доступной в Node.js
- Классы в импортах (нарушение функциональной парадигмы)

**🎭 THEATRICAL CODE - особо опасные случаи:**
- Функции импортируются и НЕ вызываются, но создают иллюзию использования
- Типы/интерфейсы импортируются, но не применяются к переменным
- Константы импортируются, но не используются в логике
- Middleware импортируется, но не подключается в pipeline

#### Фаза 5: Документирование находок

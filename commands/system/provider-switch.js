  ...
  async execute(args = [], context = {}) {
    try {
      ...
      if (availableProviders.length === 1) {
        return outputHandler.formatWarning(
          `Only one provider available: ${availableProviders[0].name}`,
        )
      }
      ...
      if (args.length > 0) {
        ...
        if (!targetProvider) {
          ...
          return outputHandler.formatError(
            `Provider '${targetProviderName}' not found. Available: ${availableNames}`,
          )
        }

        if (targetProvider.isCurrent) {
          return outputHandler.formatWarning(
            `Already using ${targetProvider.name}`,
          )
        }
        ...
        try {
          ...
          return outputHandler.formatSuccess(
            `Switched to ${targetProvider.name}`,
          )
        } catch (error) {
          spinner.stop('error')
          throw error
      ...
      if (selectedProvider.key === currentProviderKey) {
        return outputHandler.formatWarning(
          `Already using ${selectedProvider.name}`,
        )
      }
      ...
      try {
        ...
        spinner.stop('success')
        return outputHandler.formatSuccess(
          `Switched to ${selectedProvider.name}`,
        )
      } catch (error) {
        spinner.stop('error')
        throw error

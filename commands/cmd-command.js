/**
 * cmd-command.js - Command management system with modern menu-based UX
...
/**
 * CMD Command functional object (NO CLASS!)
 */
    ...
    try {
      ...
      if (context.esc && context.esc.register) {
        escHandlerId = context.esc.register(() => {
          ...
          if (escapeResolve) {
            console.log() // New line after ESC
      ...
      while (true && !isCancelled) {
        // Create escape promise for this menu interaction
        const escapePromise = new Promise(resolve => {
          escapeResolve = resolve
        })
        
        // Race between menu selection and ESC
      ...
      // Always unregister ESC handler when exiting
...
async function editCommandFields(commandData, context, mode) {
  ...
  while (true) {
    // Generate dynamic menu with inline field values and change tracking
    ...
    // Handle ESC or Back button (last item)
    ...
    // Execute field action (only for first 6 field options)
...
function generateMenuWithValues(commandData, fieldOptions, originalData = null) {
  ...
  // Add field options with values
  fieldOptions.forEach(field => {
    if (field.name === 'Save & Exit' || field.name === 'Cancel') {
      return // Skip action buttons for now
    }
...
/**
 * Edit keys with unified collection system
 */
...
/**
 * Get all existing keys from system commands and user commands
 */
function getAllExistingKeys() {
  const allKeys = []
  
  // Add system command keys
  allKeys.push(...getAllSystemCommandNames())
  
  // Add user command keys from database
...
/**
 * Validate key against global conflicts (no regex!)
 */
function validateKey(newKey, commandData) {
  ...
  // Check for invalid characters using simple string operations
  ...
  // Check against current command's keys
  if (commandData.key.includes(newKey)) {
    return "Key already exists in this command"
  }
  
  // Check against ALL existing keys (system + user commands)
...
/**
 * Add key with validation (flow function)
 */
...
/**
 * Remove keys interactively using toggle menu (flow function)
 */
async function removeKeysInteractive(commandData, context) {
  ...
  if (commandData.key.length === 1) {
    // Single key - direct confirmation
  ...
  // Multiple keys - use toggle menu with intuitive UI
  const result = await createToggleMenu(
    'Confirm removal - keys marked with ☓ will be deleted:',
    commandData.key,
    commandData.key, // Start with all keys selected (✓ = keep)
    ['Confirm', 'Back']
  )
  
  if (result.action === 'confirm') {
    const keysToRemove = result.removedItems // Items user unchecked (☓)
...
/**
 * Add model with provider selection (flow function)
 */
async function addModelProviderFlow(commandData, context) {
  // Get available providers through clean interface
  ...
  // Get models for selected provider by temporarily switching context
  const originalProvider = context.providers.getCurrent()
  let providerModels = []
  
  try {
    // Temporarily switch to selected provider to get its models
  ...
  // Select model using the same approach as model-command.js
...
/**
 * Remove models using toggle menu (flow function)
 */
async function removeModelsToggle(commandData, context) {
  ...
  if (modelDisplayNames.length === 1) {
    // Single model - direct confirmation
    const result = await createNavigationMenu(
  ...
  // Multiple models - use toggle menu with intuitive UI
  const result = await createToggleMenu(
    'Confirm removal - models marked with ☓ will be deleted:',
    modelDisplayNames,
    modelDisplayNames, // Start with all models selected (✓ = keep)
    ['Confirm', 'Back']
  )
  
  if (result.action === 'confirm') {
    const modelsToRemove = result.removedItems // Items user unchecked (☓)
    if (modelsToRemove.length > 0) {
      // Remove unchecked models by display name
...
/**
 * Remove key from command (legacy function - kept for backward compatibility)
 */
async function removeKey(commandData, context) {
  const selectedIndex = await createNavigationMenu(
  ...
  if (selectedIndex !== -1) {
    const removedKey = commandData.key.splice(selectedIndex, 1)[0]
    console.log(color.green + `Key "${removedKey}" removed` + color.reset)
  }
}

/**
 * Edit description field - direct input
 */
...
/**
 * Edit models with unified collection system
 */
...
async function handleListCommands(context) {
  try {
    ...
    // Simple pause instead of menu - no interference with command display
...
/**
 * Generate unique command ID from name (NO REGEXP!)
 */

...
export const HelpCommand = {
  ...
  async execute(args = [], context = {}) {
    try {
      ...
      const tableConfig = getConfig('UI.HELP_TABLE.COLUMN_WIDTHS')
      const separator = getConfig('UI.HELP_TABLE.SEPARATORS.COLUMN')
      const formatting = getConfig('UI.HELP_TABLE.FORMATTING')
      ...
      try {
        ...
        if (Object.keys(userCommands).length > 0) {
          ...
          const tableConfig = getConfig('UI.HELP_TABLE.COLUMN_WIDTHS')
          const separator = getConfig('UI.HELP_TABLE.SEPARATORS.COLUMN')
          const rowSep = getConfig('UI.HELP_TABLE.SEPARATORS.ROW')
          const formatting = getConfig('UI.HELP_TABLE.FORMATTING')

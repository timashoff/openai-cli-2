...
export class CommandRepository {
  ...
  async findById(id, options = {}) {
    ...
    if (!id || typeof id !== 'string') {
      throw createBaseError('Command ID must be a non-empty string', true, 400)
    }
    ...
    try {
      ...
      throw createBaseError(`Failed to find command: ${error.message}`, false, 500)
    }
  }
  ...
  async findByKeyword(keyword, options = {}) {
    ...
    if (!keyword || typeof keyword !== 'string') {
      throw createBaseError('Keyword must be a non-empty string', true, 400)
    }
  ...
  async save(id, commandData) {
    try {
      ...
      throw createBaseError(`Failed to save command: ${error.message}`, false, 500)
    }
  }
  ...
  async migrateFromInstructions(instructions) {
    if (!instructions || typeof instructions !== 'object') {
      throw createBaseError('Instructions must be an object', true, 400)
    }

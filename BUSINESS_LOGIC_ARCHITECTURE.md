...
- ✅ **ДОЛЖНЫ кешироваться** - кеш общий для всех моделей команды
- **Ключ кеша** = чистый пользовательский ввод БЕЗ команды-инструкции для LLM
- **Пример ключа**: `"как дела?"` (НЕ `"Переведи на русский: как дела?"`)
- **Кеш возвращает**: полный мультимодельный вывод со всеми моделями
...
- Ожидание ввода пользователя в prompt
...
Система проверяет наличие маркера `$$` в пользовательском вводе и заменяет его содержимым буфера обмена с валидацией размера.

#### 2.2 Обработка force flags:
Детекция флагов принудительного выполнения (`--force`, `-f`) для обхода кеша и их удаление из пользовательского ввода.

#### 2.3 Поиск команды в БД:
Поиск соответствующей команды в SQLite базе данных по ключевым словам пользователя.

#### 2.4 MCP обработка (если есть URL):
Автоматическое определение потребности в обработке веб-контента или поиска и вызов соответствующих MCP серверов.

#### 2.5 Кэш проверка:
- **Multi-provider команды**: проверка наличия кешированных ответов от всех моделей
- **Translation команды**: проверка кеша переводов по ключу пользовательского ввода

#### 2.6 Выполнение запроса:

**Multi-model processing**: Если команда настроена для работы с несколькими моделями одновременно, система использует специальный процессор для параллельного выполнения запросов и агрегации результатов.

**Single provider processing**: 
- Система проверяет наличие выбранного провайдера по ключу
- Выполняется ленивая загрузка провайдера при первом обращении через ServiceManager
- Создается потоковое соединение с возможностью отмены
- Ответ обрабатывается StreamProcessor для real-time вывода
...
Подготовленная современная архитектура с обработчиками, временно отключенная для изучения текущей системы.
...
- **utils/migration.js**: Миграция команд из конфига в базу
...
- **config/api_providers.js**: Конфигурация AI провайдеров

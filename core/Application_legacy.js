...
export class Application {
  ...
  async initialize() {
    ...
    try {
      ...
      this.requestRouter = createRouter(this.stateManager, this.cliInterface, {
  ...
  async handleUserInput(userInput) {
    ...
    try {
      ...
      await this.executeRoutedAction(routingResult, controller)
      ...
      // Reset state on error or completion
   ...
   */
  async executeRoutedAction(routingResult, controller) {
    ...
    switch (action) {
      ...
      case 'process_enhanced_input':
        ...
        await this.processAIRequest(input, command, metadata, controller)
        break
   ...
   */
  async processAIRequest(input, command = null, metadata = {}, controller) {
    // Get AI state
  ...
  async processAIStream(stream, originalInput, command) {
    ...
    try {
      ...
      if (command && cacheManager.shouldCache(command, null, false)) {
        const cacheKey = cacheManager.generateCacheKey(
          command.hasUrl ? command.originalInput : command.fullInstruction,
          command
        )
        await cacheManager.setCache(cacheKey, response, true)
      }
  ...
  getStatus() {
    return {
      ...
      router: this.requestRouter ? this.requestRouter.getRoutingStats() : null
    }
  }
}

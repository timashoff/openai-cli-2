...
export class CommandManager {
  ...
  registerCommand(command) {
    if (!(command instanceof BaseCommand)) {
      throw createBaseError('Command must be an instance of BaseCommand', true, 400)
    }
    
    if (this.commands.has(command.name)) {
      throw createBaseError(`Command ${command.name} is already registered`, true, 400)
    }
    ...
    for (const alias of command.aliases) {
      if (this.aliases.has(alias)) {
        throw createBaseError(`Alias ${alias} is already registered`, true, 400)
      }
  ...
  async executeCommand(commandName, args = [], context = {}) {
    ...
    if (!command) {
      throw createBaseError(`Command ${commandName} not found`, true, 404)
    }
    
    try {
      // Validate arguments
      if (!command.validateArgs(args)) {
        throw createBaseError(`Invalid arguments for command ${commandName}`, true, 400)
      }
